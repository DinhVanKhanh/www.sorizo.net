...
// ↑↑　<2020/10/30> <VinhDao> <追加>
     ...
     * オンラインソリクラ契約表示制御
     */
    function isLoginSerialOnlineNormal($category) {
        $ky_next_kb = GetCookie(LoginProduct, f_ky_next_kb);

        if ($category == "online") {
            // オンラインソリクラ契約者
            if ($ky_next_kb === 3) return true;
     ...
     * 引数に応じて表示するコンテンツを制御
     */
    function showContents($category) {

        // ログインシリアルNoがオンラインソリクラまたは通常ソリクラの場合
        if (isLoginSerialOnlineNormal($category)) return "";
        return "display: none;";
    }
    ...
    // ↓↓　<2020/10/29> <VinhDao> <cookie>
        ...
        foreach ($ky as $item) {
            if (is_array($item) and is_array($item["ky_prod"]) and is_array($item["ky_prod"][0]["sral"])) {
                ...
                if ($sral_no == $serial_no) {
                    $ky_next_kb = $item["ky_next_kb"];
                    $serialLogin = [f_serialno => $serial_no, f_expires  => date('Y/m/d', $ky_e_ymd), f_userpref => $pref_nm, f_ky_next_kb => $ky_next_kb];
                }
            }
        }
        ...
        foreach($sral as $item) {
            ...
            {
                $getListSralProd[$key] = [f_serialno => $item, f_expires  => "2099/01/01", f_ky_next_kb => $ky_next_kb];
            }
        };
        ...
        if ( empty($serialLogin) ) {
            $serialLogin = [f_serialno => $serial_no, f_expires  => "2099/01/01", f_userpref => $pref_nm, f_ky_next_kb => $ky_next_kb];
        }
